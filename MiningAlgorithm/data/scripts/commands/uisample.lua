...
function execute(sender, commandName, action)
    ...
    if action == "on" or action == "enable" then
        ...
        for _, scriptPath in pairs(scripts) do
            if scriptPath == "../entity/miningalgocontroller.lua" then
                hasScript = true
                break
            end
        end
        ...
        if not hasScript then
            entity:addScriptOnce("../entity/miningalgocontroller.lua")
    ...
    elseif action == "off" or action == "disable" then
        ...
        entity:invokeFunction("../entity/miningalgocontroller.lua", "disable")
        entity:setValue("uisample_initialized", nil)

        for i = 1, scriptCount do
            entity:removeScript("../entity/miningalgocontroller.lua")
        end
